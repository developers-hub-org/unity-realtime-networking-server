...
namespace DevelopersHub.RealtimeNetworking.Server.Scripts
{
    public class Email
    {
        ...
        public static bool DemoSendEmailVerificationCode(string code, string email, int expiration)
        {
            string mail = email_verification_code_template;
            mail = mail.Replace("[company_logo_url]", "https://developershub.org/wp-content/uploads/2023/04/developershub_logo_black_512x512.png");
            ...
            mail = mail.Replace("[remained_time]", expiration + " minutes");
            mail = mail.Replace("[copyright_footer]", "© " + DateTime.UtcNow.Year.ToString() + " " + name + " , Torento, Canada");
            return Send(email, "Verification Code", mail);
        }
